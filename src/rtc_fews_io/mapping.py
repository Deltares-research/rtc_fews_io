...
class DataConfig:
    ...
    def parameter(
        ...
        return self._parameter_by_external_id[
            _parameter_key(
                model_id=model_id, location_id=location_id, parameter_id=parameter_id
            )
        ]
    ...
    def _series_id_from_input(self, value: Any) -> str:
        ...
        raise TypeError(
            f"Cannot derive FEWS time series identifiers from {type(value)!r}."
        )


def _series_key(
    location_id: str, parameter_id: str, qualifier_ids: tuple[str, ...]
) -> str:
...
def _first_child(element: ET.Element, name: str) -> ET.Element | None:
    return next(
        (child for child in _children(element) if _local_name(child.tag) == name), None
    )
